#!/bin/bash
# Arbitrum Node - Final Working Configuration
...
echo "Starting Arbitrum node with final working configuration..."

# Remove any existing container
docker rm -f arbitrum-node 2>/dev/null || true

# Run Arbitrum node using official configuration from docs
exec docker run --rm \
    --name arbitrum-node \
    --network host \
