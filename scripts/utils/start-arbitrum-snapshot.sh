#!/bin/bash
# Arbitrum Node - Snapshot Initialization
...
echo "Starting Arbitrum node with snapshot initialization..."
echo "This may take time as it downloads the snapshot..."

# Remove any existing container
docker rm -f arbitrum-node 2>/dev/null || true

# Run Arbitrum node with snapshot initialization
exec docker run --rm \
    --name arbitrum-node \
    --network host \
    -v "$DATA_DIR":/data \
